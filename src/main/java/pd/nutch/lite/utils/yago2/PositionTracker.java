 ...
 * changes within a String, e.g. caused through normalization etc. This allows
 * for instance, given a position int the normalized string to get the
 * corresponding position in the original non-normalized string
 *
 *
 *
 * backward position tracker - tracking several replacement/text changes
 * allowing to trace a position in the modified text back to the corresp.
 * position in the original text for the other direction see
...
public class PositionTracker {
  ...
  /**
   * Closes the current changing run by Merging new position changes into the
   * existing position change map after each round (one round=consecutive
   * changes along the text) you need to call closeRun() before submitting more
   * position changes from a new round, i.e. whenever you passed the string to
   * be modified once call closeRun() before starting to run over the string
   * again with more replacements Do this every time you ran once over the text
   * making changes to be tracked
   */
  ...
  /**
   * Merges new position changes (given with the inversed old2new mapping) into
   * the existing position change ma
   *
   * @param pos
   * @return p
   */
  ...
  /**
   * forward position change tracking - keeping track of several rounds of text
   * modifications allowing to trace a position in the original text along the
   * modifications to the corresp. position in the modified text after each
   * round (one round=consecutive changes along the text) you need to call
   * closeRun() before submitting more position changes from a new round, i.e.
   * whenever you passed the string to be modified once call closeRun() before
   * starting to run over the string again with more replacements REMARK: NOT
   * TESTED WITH MORE THAN ONE ROUND! may be ERRORNOUS with multiple rounds ->
   * use with care (works with a single round though)
   *
   * @author smetzger
   *
   */
  public static class ForwardPositionTracker {
    ...
    /**
     * Closes the current changing run by Merging new position changes into the
     * existing position change map Do this every time you ran once over the
     * text making changes to be tracked
     */
    public void closeRun() {
      if (positionChanges.isEmpty()) {
        return;
    ...
    /**
     * tells whether a position in the original stream has been cut away by some
     * change operation, such that translating it usually would make not to much
     * sense
     *
     * @param pos
     * @return true, iff the given position has been cut away, false otherwise
     *         (i.e. false if it should be mappable) TODO: current version ONLY
     *         WORKS SECURELY WHEN THERE IS ONLY ONE POSITION CHANGE RUN WITHOUT
     *         OVERLAPPING CHANGES! as soon as there are more than one change
     *         runs, or changes that overlap, we would need to check all
     *         following changes instead of only the next one
     */
     ...
     *
     * TODO: current version ONLY WORKS SECURELY WHEN THERE IS ONLY ONE POSITION
     * CHANGE RUN WITHOUT OVERLAPPING CHANGES! as soon as there are more than
     * one change runs, or changes that overlap, we would need to check all
     * following changes instead of only the next one
     *
     * @param pos
     * @return
