 ...
 */
package pd.nutch.lite.utils.ixatools.pos.lemma;
...
import pd.nutch.lite.utils.ixatools.pos.pos.StringUtils;
...
public class MultiWordMatcher {
  ...
  private void loadDictionary(final Properties props) throws IOException {
    ...
    final BufferedReader breader = new BufferedReader(new InputStreamReader(dictInputStream, Charset.forName("UTF-8")));
    String line;
    while ((line = breader.readLine()) != null) {
      final String[] lineArray = tabPattern.split(line);
      if (lineArray.length == 4) {
        final Matcher lineMatcher = linePattern.matcher(lineArray[0].toLowerCase());
  ...
  private final InputStream getMultiWordDict(final String lang) {
    ...
    if (lang.equalsIgnoreCase("es")) {
      dict = getClass().getResourceAsStream("/lemmatizer-dicts/freeling/es-locutions.txt");
    }
    if (lang.equalsIgnoreCase("gl")) {
      dict = getClass().getResourceAsStream("/lemmatizer-dicts/ctag/gl-locutions.txt");
    }
    return dict;
  }
  ...
  public final String[] getTokensWithMultiWords(final String[] tokens) {
    ...
    for (final Span mwSpan : multiWordSpans) {
      ...
      final String multiWord = Joiner.on("#").join(tokenList.subList(fromIndex, toIndex));
    ...
    for (int offsetFrom = 0; offsetFrom < tokens.length; offsetFrom++) {
      ...
      for (int offsetTo = offsetFrom; offsetTo < tokens.length; offsetTo++) {
        ...
        if (lengthSearching > getMaxTokenCount()) {
          ...
          final String entryForSearch = StringUtils.getStringFromTokens(tokensSearching);
          final String entryValue = dictionary.get(entryForSearch.toLowerCase());
          if (entryValue != null) {
