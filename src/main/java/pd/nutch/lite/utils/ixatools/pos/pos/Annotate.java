 ...
 */
package pd.nutch.lite.utils.ixatools.pos.pos;
...
import pd.nutch.lite.utils.ixatools.pos.lemma.DictionaryLemmatizer;
import pd.nutch.lite.utils.ixatools.pos.lemma.MorfologikLemmatizer;
import pd.nutch.lite.utils.ixatools.pos.lemma.MultiWordMatcher;
import pd.nutch.lite.utils.ixatools.pos.pos.dict.DictionaryTagger;
import pd.nutch.lite.utils.ixatools.pos.pos.dict.MorfologikMorphoTagger;
...
public class Annotate {
  ...
  private String mapGalicianTagSetToKaf(final String postag) {
    if (postag.startsWith("R")) {
      ...
      return "C"; // conjunction
    } else if (postag.startsWith("D") || postag.startsWith("G") || postag.startsWith("X") || postag.startsWith("Q") || postag.startsWith("T") || postag.startsWith("I") || postag.startsWith("M")) {
      return "D"; // det predeterminer
    } else if (postag.startsWith("A")) {
      return "G"; // adjective
    } else if (postag.startsWith("NC")) {
  ...
  public final void annotatePOSToKAF(final KAFDocument kaf) {
    ...
    for (final List<WF> wfs : sentences) {
      ...
      if (this.multiwords) {
        final String[] multiWordTokens = this.multiWordMatcher.getTokensWithMultiWords(tokens);
      ...
      for (int i = 0; i < morphemes.size(); i++) {
        ...
        if (this.dictag) {
          final String dictPosTag = this.dictMorphoTagger.tag(morphemes.get(i).getWord(), morphemes.get(i).getTag());
        ...
        final String lemma = this.dictLemmatizer.lemmatize(morphemes.get(i).getWord(), morphemes.get(i).getTag());
   ...
   */
  private void getMultiWordSpans(final String[] tokens, final List<WF> wfs, final List<ixa.kaflib.Span<WF>> tokenSpans) {
    final Span[] multiWordSpans = this.multiWordMatcher.multiWordsToSpans(tokens);
    int counter = 0;
    for (final Span mwSpan : multiWordSpans) {
      ...
      final List<WF> wfTargets = wfs.subList(mwSpan.getStart(), mwSpan.getEnd());
      final ixa.kaflib.Span<WF> multiWordSpan = KAFDocument.newWFSpan(wfTargets);
