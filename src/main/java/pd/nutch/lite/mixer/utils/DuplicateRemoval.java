package pd.nutch.lite.mixer.utils;
...
import pd.nutch.lite.model.ListTerm;
...
public class DuplicateRemoval {
  ...
  /**
   * First Duplicate removal Step, here a comparison using equalsIgnoreCase from
   * String class is done
   *
   * @param pTermList
   *          - a list of list containing terms extracted by the term extraction
   *          methods
   *
  ...
  public static List<Term> simpleDuplicateRemoval(List<ListTerm> pTermList) {
    ...
    // System.out.println(pTermList1.getName()+"
    // "+pTermList1.getTermList().size());
  ...
  public static void topicDuplicateRemoval(Document pDoc) {
    ...
    // TODO: this step could be combined with disambiguationRemoval? (check
    // topic.getGoodSense)
  ...
  // }
  /**
   * If a term is not disambiguated correctly, aka in the disambiguation process
   * it has not be mapped to an id within a knowledge source it is deleted
   *
   * @param pDoc
